...
// ReSharper disable once InconsistentNaming
        ...
        public bool ShouldDropInbound(Address remoteAddress, object instance, string debugMessage)
        {
            ...
            if (mode is Drop)
            {
                ...
                {
                    if(_shouldDebugLog) _log.Debug("Dropping inbound [{0}] for [{1}] {2}", instance.GetType(), remoteAddress, debugMessage);
                    return true;
                }
            }
        ...
        public bool ShouldDropOutbound(Address remoteAddress, object instance, string debugMessage)
        {
            ...
            if (mode is Drop)
            {
                ...
                {
                    if (_shouldDebugLog) _log.Debug("Dropping outbound [{0}] for [{1}] {2}", instance.GetType(), remoteAddress, debugMessage);
                    return true;
                }
            }
