...
namespace Akka.Cluster.Tests.Performance.Serialization
{
    public class ClusterMessageSerializerSpec
    {
        ...
        internal Lazy<InternalClusterAction.Welcome> WelcomeMessage { get; } = new Lazy<InternalClusterAction.Welcome>(
            ...
            {
                var member1 =
                    new Member(new UniqueAddress(new Address("akka.tcp", "system", "some.host.org", 4718), 34), 1,
                        MemberStatus.Joining, ImmutableHashSet<string>.Empty);
                var member2 =
                    new Member(new UniqueAddress(new Address("akka.tcp", "system", "some.host.org", 4710), 35), 1,
                        MemberStatus.Joining, ImmutableHashSet<string>.Empty);
